# Decision Transformer in PyBullet Gym Environments-V0 
For this experiment, we would like to test the performance of our Decision Transformer in PyBullet Gym Environment-V0 using the d4rl-pybullet library [d4rl-pybullet](https://github.com/takuseno/d4rl-pybullet) from [takuseno](https://github.com/takuseno)'s repository. The original idea was to utilize the latest Mujoco environments from Gymnasium OpenAI, but we didn't find many datasets available and also encountered several problems installing the library due to deprecated dependencies and license issues. Therefore, we opted for the mentioned dataset and its associated environment since it suited our needs.
